...
current working directory.  If you try to run a command and it's not in the docker container, do not look elsewhere and do not try to install it.  In that case, stop and ask the developer to modify the docker container.

## Tracking Context

Whenever you run, create a hidden directory .claude-tracking and inside that create a file called context.md and summarize your context.  This is not for normal use, but is intended to help the developer regain context if they have to stop you to add functionality to the container.  On restart, the developer can ask you to read that file to regain context.  Always add the folder .claude-tracking to .gitignore.
